...
const SensorCard = ({ title, value, unit, icon: Icon, status, statusColor }: { 
  ...
  >
    ...
    <div className="flex items-baseline gap-2">
      <span className="text-5xl font-bold font-headline text-white">{value}</span>
      {unit && <span className="text-xl font-light text-slate-400">{unit}</span>}
    </div>
            ...
            <section className="bg-surface-low rounded-xl p-8 border border-outline-variant/10 shadow-2xl">
              <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                <div className="space-y-2 text-center md:text-right">
                  <h2 className="text-2xl font-bold font-headline text-white flex items-center justify-center md:justify-start gap-3">
              ...
              {[1, 2, 3, 4].map((id) => (
                <div key={id} className="relative aspect-video bg-surface-lowest rounded-2xl border border-outline-variant/10 overflow-hidden group shadow-2xl">
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-b from-transparent to-black/40 z-10">
                    ...
                    <p className="text-slate-500 font-mono tracking-[0.5em] text-xs uppercase">كاميرا {id} • مشفر</p>
                  </div>
    ...
    <div className="min-h-screen flex flex-col" dir="rtl">
      ...
      <header className="fixed top-0 w-full z-50 flex justify-between items-center px-6 h-16 bg-surface border-b border-outline-variant/15">
        <div className="flex items-center gap-4">
          ...
          <span className="text-xl font-bold tracking-tighter text-primary-container font-headline">مراقبة النظام</span>
      ...
      <div className="flex flex-1 pt-16">
        ...
        {/* Sidebar Navigation */}
        <aside className={`
          fixed right-0 top-0 h-full flex flex-col pt-20 pb-8 bg-surface-low w-64 shadow-[-40px_0_60px_-15px_rgba(0,0,0,0.3)] z-40
